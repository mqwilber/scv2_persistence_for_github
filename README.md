...
- `data/`
	- `deer_data/collar_data_01252025.csv`: Individual information on deer as used in this paper.
	- `spatial_data/`: Contains shape files used in the analysis
- `results/`
	- `host_uds/`
		- `all_host_uds.zip`: A zip file that contains the empirical data for all deer utilization distributions used in this study. UNZIP THIS FILE TO RUN THE MODEL SCRIPTS.  These are the UDs generated by `generate_UDs.py` Note, these UDs were built in the `movement_clean` environment specified by `environment.yml` so you will need to be in this Python environment to extract them.
